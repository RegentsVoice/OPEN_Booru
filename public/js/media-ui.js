...
export function addTagFromInput() {
    ...
    if (!inputValue) { loadMedia(0); return; }
    const tagsArray = inputValue.split(',').map(t => t.trim()).filter(t => t);
    const currentTags = parseTagsToArray(state.tags);
    const combined = [...currentTags, ...tagsArray];
    ...
    statusPollingInterval = setInterval(async () => {
        try {
            ...
            if (status.stage === 'container') {
                ...
                uploadSubmitBtn.textContent = t('close');
                uploadSubmitBtn.disabled = false;
