...
function bindEvents() {
    ...
    // Close suggestions when input loses focus (delay so mousedown on item still works)
    ...
    /** Extract first http(s) URL from a drag/drop DataTransfer */
    function urlFromDataTransfer(dt) {
        ...
        // Sometimes browsers put the link in text/html
    ...
    function applyDroppedImportUrl(url) {
        ...
        // Auto-fetch preview like after manual paste + Enter
    ...
    dropArea.addEventListener('drop', (e) => {
        ...
        // Prefer files; otherwise treat as dropped URL
    ...
    // Drop URL anywhere on the upload modal (not only the input)
    ...
    if (uploadModalContent) {
        uploadModalContent.addEventListener('dragover', (e) => {
            // Only highlight/allow when dragging a URL (no files), or always allow preventDefault so drop fires
        ...
        uploadModalContent.addEventListener('drop', (e) => {
            uploadModalContent.classList.remove('url-dragover');
            // If drop target is the file dropArea, its own handler already ran (bubble) —
            // stop here only for URL without files when not handled as file
            if (e.dataTransfer?.files?.length) {
                // Let file drops on dropArea work; if dropped outside dropArea, also accept files
