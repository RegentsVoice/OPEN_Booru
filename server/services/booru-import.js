...
/** @type {Map<string, { tempPath: string, originalName: string, fileSize: number, tags: string[], mediaType: string, sourceUrl: string, created: number, userId: number }>} */
export const importTemp = new Map();

const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36';

/** Canonical site keys used in credentials table */
 ...
 * @param {Record<string, { user_id?: string, account_id?: string, login?: string, api_key?: string }>|null} credentialsBySite
 */
...
export async function resolveBooruUrl(inputUrl, credentialsBySite = null) {
    ...
    if (gelbooruHosts.some(h => host === h || host.endsWith('.' + h))) {
        ...
        // Realbooru usually works without auth; gelbooru/rule34 require keys
        ...
        try {
            ...
            // Some APIs return { "@attributes": ..., post: [...] } or empty array
            const post = list[0] || (data && data.file_url ? data : null);
            if (!post) {
                // Fallback: scrape HTML page for media URL
                try {
            ...
            fileUrl = preferMp4OverWebm(normalizeCdnUrl(fileUrl));
            // For videos, HTML page often has better mp4 source — try if API gave webm or suspicious
    ...
    // --- Philomena family (Derpibooru, Furbooru, …) ---
    ...
    if (philo || siteId === 'derpibooru' || siteId === 'furbooru' || siteId === 'ponybooru') {
        ...
        // filter_id=56027 is Derpibooru "Everything"; Furbooru may ignore unknown filters
        const qs = new URLSearchParams();
        // Derpibooru "Everything" filter; other Philomena instances may ignore unknown ids
...
export async function downloadImportToTemp(resolved, userId) {
    ...
    // Hotlink protection: follow one redirect with referer if needed
    if (res.status >= 300 && res.status < 400) {
        const loc = res.headers.get('location');
        if (loc) {
            const next = loc.startsWith('http') ? loc : new URL(loc, downloadUrl).href;
            // Skip hotlink.php interstitial — prefer direct CDN with referer
    ...
    // Stream to disk to handle large videos without truncating
    ...
    // Basic magic-byte sanity for common video/image types
...
/** Detach from map without deleting the file (processMedia will unlink). */
export function releaseImportTemp(tempId) {
    importTemp.delete(tempId);
}

/**
 * Lightweight API check that credentials work.
...
export async function verifyBooruCredentials(site, { account_id, login, api_key } = {}) {
    ...
    try {
        if (site === 'gelbooru' || site === 'rule34' || site === 'realbooru') {
            ...
            // Success: JSON array or object
            if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
                try {
                    const data = JSON.parse(trimmed);
                    // Rule34 often returns error as JSON string message
            ...
            if (!res.ok) {
                ...
                return {
                    ...
                    // allow client to offer "save anyway" when site blocks datacenter IPs etc.
        ...
        if (site === 'derpibooru' || site === 'furbooru' || site === 'ponybooru') {
            ...
            if (!res.ok) {
                // some instances may not expose filters endpoint — try images
